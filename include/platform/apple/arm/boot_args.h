...
struct {
    uint16_t revision;
    uint16_t version;
    uint32_t virtual_base;     /* Kernel address - Virtual */
    uint32_t physical_base;    /* Kernel address - Physical */
    uint32_t memory_size;      /* I don't actually know what this referse to, maximum Virtual mem space? */
    uint32_t top_of_kernel_data; /* "Highest physical address used in kernel data area", This essentially just means 'where does our __DATA segment end?' */
    ...
    uint32_t boot_flags;
    uint32_t real_memory_size; /* Physical RAM size, I think. */
